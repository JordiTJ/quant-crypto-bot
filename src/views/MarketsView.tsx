...
export const MarketsView: React.FC<MarketsViewProps> = ({ markets, onSelectMarket }) => {
  ...
  const filtered = markets
    .filter(m => {
      const matchSearch = 
        ...
        (m.coinName && m.coinName.toLowerCase().includes(searchTerm.toLowerCase()));
      const matchCategory = selectedCategory === 'ALL' || m.category === selectedCategory;
      if (filterEligibleOnly) return matchSearch && matchCategory && m.isEligible;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-lg p-4">
        <div>
          <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
            ...
            Uitgebreid Markt Universum & Screening
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            Realtime rangschikking van de meest relevante crypto activa gesorteerd op volume, liquiditeit, marktregime en trend momentum.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Zoek coin (bijv. TAO, SUI, BTC)..."
              value={searchTerm}
      ...
      <div className="flex flex-wrap items-center gap-2 bg-slate-900/60 p-2 rounded-lg border border-slate-800">
        <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider px-2">Sectoren:</span>
        {categories.map(cat => (
          <button
            key={cat.id}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-1.5">
                            <span className="text-slate-100 font-bold">{m.symbol}</span>
